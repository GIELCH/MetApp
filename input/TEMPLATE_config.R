list_transfo = "input/list_transfo.tsv"
bool_phase_1 = TRUE
bool_phase_2 = FALSE
lib_perso = ""  # give the personnal R library if needed
nb_transformation = 3
min_peak_intensity = 5e5
mz_ppm = 10  # confidence m/z to search chromatogram
rt_windows = 5  # windows size to consider a peak in chromatogram
nb_scan = 2  # minimum number of scans in chromatogram peaks
wdw_mz_ms2 = 1  # total window width
minimum_mz = 200 # Minimum M/Z value of a metabolite to be detected
